...
@implementation UIFont (ZGAdd)

- (BOOL)isBold {
    return [self matchFontDescriptorTrait:UIFontDescriptorTraitBold];
}

- (BOOL)isItalic {
    return [self matchFontDescriptorTrait:UIFontDescriptorTraitItalic];
}

- (BOOL)isMonoSpace {
    return [self matchFontDescriptorTrait:UIFontDescriptorTraitMonoSpace];
}

- (BOOL)matchFontDescriptorTrait:(UIFontDescriptorSymbolicTraits)trait {
    if (![self respondsToSelector:@selector(fontDescriptor)]) return NO;
    return (self.fontDescriptor.symbolicTraits & trait) > 0;
}
...
- (nullable UIFont *)fontWithNormal {
    if (![self respondsToSelector:@selector(fontDescriptor)]) return self;
