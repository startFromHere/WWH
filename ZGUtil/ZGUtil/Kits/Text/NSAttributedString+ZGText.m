//
//  NSAttributedString+ZGText.m
//  ZGUtil
//
//  Created by 刘涛 on 2022/10/10.
//

#import "NSAttributedString+ZGText.h"
#import "YYKitMacro.h"
#import "UIDevice+ZGAdd.h"
#import "UIFont+ZGAdd.h"
#import "NSParagraphStyle+ZGAdd.h"
#import "ZGTextArchiver.h"
#import "ZGTextRunDelegate.h"
//#import "ZGAnimatedImageView.h"
#import "ZGTextUtilities.h"
#import <CoreFoundation/CoreFoundation.h>


YYSYNTH_DUMMY_CLASS(NSAttributedString_ZGAdd)

@implementation NSAttributedString (ZGText)

- (NSData *)archiveToData {
    NSData *data = nil;
    @try {
        data = [ZGTextArchiver archivedDataWithRootObject:self];
    }
...
+ (instancetype)unarchiveFromData:(NSData *)data {
    ...
    @try {
        one = [ZGTextUnarchiver unarchiveObjectWithData:data];
    } @catch (NSException *exception) {
        NSLog(@"%@", exception);
    }
    return one;
...
- (ZGTextShadow *)textShadow {
    return [self textShadowAtIndex:0];
}

- (ZGTextShadow *)textShadowAtIndex:(NSUInteger)index {
    return [self attribute:ZGTextShadowAttributeName atIndex:index];
}

- (ZGTextShadow *)textInnerShadow {
    return [self textInnerShadowAtIndex:0];
}

- (ZGTextShadow *)textInnerShadowAtIndex:(NSUInteger)index {
    return [self attribute:ZGTextInnerShadowAttributeName atIndex:index];
}

- (ZGTextDecoration *)textUnderline {
    return [self textUnderlineAtIndex:0];
}

- (ZGTextDecoration *)textUnderlineAtIndex:(NSUInteger)index {
    return [self attribute:ZGTextUnderlineAttributeName atIndex:index];
}

- (ZGTextDecoration *)textStrikethrough {
    return [self textStrikethroughAtIndex:0];
}

- (ZGTextDecoration *)textStrikethroughAtIndex:(NSUInteger)index {
    return [self attribute:ZGTextStrikethroughAttributeName atIndex:index];
}

- (ZGTextBorder *)textBorder {
    return [self textBorderAtIndex:0];
}

- (ZGTextBorder *)textBorderAtIndex:(NSUInteger)index {
    return [self attribute:ZGTextBorderAttributeName atIndex:index];
}

- (ZGTextBorder *)textBackgroundBorder {
    return [self textBackgroundBorderAtIndex:0];
}

- (ZGTextBorder *)textBackgroundBorderAtIndex:(NSUInteger)index {
    return [self attribute:ZGTextBackedStringAttributeName atIndex:index];
}
...
- (CGAffineTransform)textGlyphTransformAtIndex:(NSUInteger)index {
    NSValue *value = [self attribute:ZGTextGlyphTransformAttributeName atIndex:index];
...
- (NSString *)plainTextForRange:(NSRange)range {
    ...
    [self enumerateAttribute:ZGTextBackedStringAttributeName inRange:range options:kNilOptions usingBlock:^(id value, NSRange range, BOOL *stop) {
        ZGTextBackedString *backed = value;
...
+ (NSMutableAttributedString *)attachmentStringWithContent:(id)content
    ...
    NSMutableAttributedString *atr = [[NSMutableAttributedString alloc] initWithString:ZGTextAttachmentToken];
    
    ZGTextAttachment *attach = [ZGTextAttachment new];
...
+ (NSMutableAttributedString *)attachmentStringWithContent:(id)content
                                            ...
                                            attachmentSize:(CGSize)attachmentSize
                                               alignToFont:(UIFont *)font
                                                 alignment:(ZGTextVerticalAlignment)alignment {
    NSMutableAttributedString *atr = [[NSMutableAttributedString alloc] initWithString:ZGTextAttachmentToken];
    ...
    switch (alignment) {
        case ZGTextVerticalAlignmentTop: {
        ...
        case ZGTextVerticalAlignmentCenter: {
        ...
        case ZGTextVerticalAlignmentBottom: {
...
//
//    ZGTextAttachment *attachment = [ZGTextAttachment new];
...
- (BOOL)canDrawWithUIKit {
    ...
    dispatch_once(&onceToken, ^{
        ...
        [failSet addObject:ZGTextShadowAttributeName];
        [failSet addObject:ZGTextInnerShadowAttributeName];
        [failSet addObject:ZGTextUnderlineAttributeName];
        [failSet addObject:ZGTextStrikethroughAttributeName];
        [failSet addObject:ZGTextBorderAttributeName];
        [failSet addObject:ZGTextBackgroundBorderAttributeName];
        [failSet addObject:ZGTextBlockBorderAttributeName];
        [failSet addObject:ZGTextAttachmentAttributeName];
        [failSet addObject:ZGTextHighlightAttributeName];
        [failSet addObject:ZGTextGlyphTransformAttributeName];
    });
...
@implementation NSMutableAttributedString (ZGText)
...
- (void)setTextBackedString:(ZGTextBackedString *)textBackedString range:(NSRange)range {
    [self setAttribute:ZGTextBackedStringAttributeName value:textBackedString range:range];
}

- (void)setTextBinding:(ZGTextBinding *)textBinding range:(NSRange)range {
    [self setAttribute:ZGTextBindingAttributeName value:textBinding range:range];
}

- (void)setTextShadow:(ZGTextShadow *)textShadow range:(NSRange)range {
    [self setAttribute:ZGTextShadowAttributeName value:textShadow range:range];
}

- (void)setTextInnerShadow:(ZGTextShadow *)textInnerShadow range:(NSRange)range {
    [self setAttribute:ZGTextInnerShadowAttributeName value:textInnerShadow range:range];
}

- (void)setTextUnderline:(ZGTextDecoration *)textUnderline range:(NSRange)range {
    [self setAttribute:ZGTextUnderlineAttributeName value:textUnderline range:range];
}

- (void)setTextStrikethrough:(ZGTextDecoration *)textStrikethrough range:(NSRange)range {
    [self setAttribute:ZGTextStrikethroughAttributeName value:textStrikethrough range:range];
}

- (void)setTextBorder:(ZGTextBorder *)textBorder range:(NSRange)range {
    [self setAttribute:ZGTextBorderAttributeName value:textBorder range:range];
}

- (void)setTextBackgroundBorder:(ZGTextBorder *)textBackgroundBorder range:(NSRange)range {
    [self setAttribute:ZGTextBackgroundBorderAttributeName value:textBackgroundBorder range:range];
}

- (void)setTextAttachment:(ZGTextAttachment *)textAttachment range:(NSRange)range {
    [self setAttribute:ZGTextAttachmentAttributeName value:textAttachment range:range];
}

- (void)setTextHighlight:(ZGTextHighlight *)textHighlight range:(NSRange)range {
    [self setAttribute:ZGTextHighlightAttributeName value:textHighlight range:range];
}

- (void)setTextBlockBorder:(ZGTextBorder *)textBlockBorder range:(NSRange)range {
    [self setAttribute:ZGTextBlockBorderAttributeName value:textBlockBorder range:range];
}

- (void)setTextRubyAnnotation:(ZGTextRubyAnnotation *)ruby range:(NSRange)range {
    if (kiOS8Later) {
...
- (void)setTextGlyphTransform:(CGAffineTransform)textGlyphTransform range:(NSRange)range {
    ...
    [self setAttribute:ZGTextGlyphTransformAttributeName value:value range:range];
}
...
- (void)setTextHighlightRange:(NSRange)range
              ...
              backgroundColor:(UIColor *)backgroundColor
                    ...
                    tapAction:(ZGTextAction)tapAction
              longPressAction:(ZGTextAction)longPressAction {
    ZGTextHighlight *highlight = [ZGTextHighlight highlightWithBackgroundColor:backgroundColor];
...
+ (NSArray *)allDiscontinuousAttributeKeys {
    ...
    dispatch_once(&onceToken, ^{
        keys = @[(id)kCTSuperscriptAttributeName,
                 ...
                 ZGTextBackedStringAttributeName,
                 ZGTextBindingAttributeName,
                 ZGTextAttachmentAttributeName].mutableCopy;
        if (kiOS8Later) {
